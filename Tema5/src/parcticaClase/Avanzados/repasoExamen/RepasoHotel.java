package parcticaClase.Avanzados.repasoExamen;
...
public class RepasoHotel {
    ...
    public static Map<String, Double> calcularFacturas(Map<String, Map<Integer, Integer>> registros, Map<Integer, Double> precios) {
        ...
        for (String dniCliente : registros.keySet()) {
            Map<Integer, Integer> ocupacionPorHabitacion = registros.get(dniCliente);
            double total = 0;
            for (Integer habitacion : ocupacionPorHabitacion.keySet()) {
                int noches = ocupacionPorHabitacion.get(habitacion);
                double precio = precios.get(habitacion);
                total += precio * noches;
            }
            resultado.put(dniCliente, total);
        }
        return resultado;
    }
    ...
    public static Map<Integer, Integer> calcularOcupacionPorHabitacion(Map<String, Map<Integer, Integer>> registros) {
        ...
        for(String key : registros.keySet()) {
            Map<Integer, Integer> ocupacionPorHabitacion = registros.get(key);
            for(Integer habitacion : ocupacionPorHabitacion.keySet()) {
                int noches = ocupacionPorHabitacion.get(habitacion);
