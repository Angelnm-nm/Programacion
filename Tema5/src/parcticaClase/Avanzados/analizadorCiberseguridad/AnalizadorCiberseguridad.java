...
public class AnalizadorCiberseguridad {
    ...
    public static void main(String[] args) {
        ...
        for (Log l : logsUnicos){
            ...
            int ocuurencias = ataquesPorIP.getOrDefault(ip,0);
            ataquesPorIP.put(ip,ocuurencias + 1);
        }
        ...
        List<Log> critico = new ArrayList<>();
        List<Log> medio = new ArrayList<>();
        List<Log> bajo = new ArrayList<>();
        for (Log log : logsUnicos){
            String zona = log.ubicacion;
            if (zona.equals("Servidores") ){
                critico.add(log);
            }
            if (zona.equals("Oficinas") ){
                medio.add(log);
            }
            if (zona.equals("Parking") ){
                bajo.add(log);
            }
        }
        for (Log log : logsUnicos){
            String zona = log.ubicacion;
            if (zona.equals("Servidores") ){
                reporteRiesgo.putIfAbsent(nivelRiesgoZona.get("Servidores"), critico);
            }
            if (zona.equals("Oficinas") ){
                reporteRiesgo.putIfAbsent(nivelRiesgoZona.get("Oficinas"), medio);
            }
            if (zona.equals("Parking") ){
                reporteRiesgo.putIfAbsent(nivelRiesgoZona.get("Parking"), bajo);
            }
        }
        System.out.println(reporteRiesgo.toString());
